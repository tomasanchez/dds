...
public class EstadoTest {
    ...
    public void estadoNuevo() {
        Estado nuevo = new Estado(Status.NUEVO, "Nuevo", "No modifica el Precio Base", (p, d) -> p);
        Assertions.assertEquals(25.5, nuevo.obtenerPrecio(25.5, 10));
    }

    @Test
    public void estadoLiquidacion() {
        Estado liquidacion = new Estado(Status.LIQ, "Liquidacion", "50% off", (p, d) -> p * 0.5);
        Assertions.assertEquals(10, liquidacion.obtenerPrecio(20, 0));
    }

    @Test
    public void estadoPromocion() {
        Estado promocion = new Estado(Status.PROMO, "Promoción", "Descuento ingresado", (p, d) -> (p - d));
